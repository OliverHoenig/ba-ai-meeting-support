...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}
    ...
    costly for a live meeting. Quadratic aging and a post-speech smoothstep keep the original score
...
original FIM score and applies a cheap, deterministic reweighting under two assumptions. First, a
thought becomes a poorer description of the current state as the conversation moves on, because later
information was not available when it was scored; its priority should fall with age. Second, once a
thought has been spoken, the motivation to say it again is initially very low, but may recover if the
group does not take it up. These two effects address different failure modes, so they multiply.
With \(s_0\) the base FIM score, \(t\) the age in seconds, and \(c\) a cooldown factor,

\begin{equation}
  s_{\mathrm{aged}}(t)
  =\max\bigl(0,\, s_0\bigl(1-(t/H)^{2}\bigr)\bigr),
  \qquad
  s_{\mathrm{eff}}
...
\paragraph{Why a quadratic.}
A linear decay \(s_0(1-t/H)\) starts eroding the score immediately. An exponential decay discounts
most steeply in the first seconds, i.e.\ while the thought is still most likely to be valid. The
downward parabola is the unique quadratic that starts at \(s_0\) with a horizontal tangent and hits
zero at a finite horizon \(H\): a fresh thought keeps nearly its full score for a short window, then
relevance falls at an increasing rate as new information accumulates
(\autoref{fig:im-dynamics}, panel~a). Once the score is below the utterance threshold \(\tau\), the finite
root also clears the thought from the buffer, rather than leaving a residual that could later
interact with cooldown recovery.
...
\begin{equation}
  H=\frac{t_{\mathrm{hold}}}{\sqrt{1-\tau/s_{\mathrm{hold}}}}.
  \label{eq:im-horizon}
\end{equation}
...
only design knob for temporal sensitivity.
...
seconds since articulation,

\begin{equation}
  c=\mathrm{smoothstep}(t_{\mathrm{spk}}/60),
  \qquad
  \mathrm{smoothstep}(x)=
  \begin{cases}
    0 & x\le 0,\\
...
could reappear within seconds---and would kink at 60\,s (\autoref{fig:im-dynamics}, panel~b). The
flat start enforces a true pause; the flat end hands control back to aging without a jump in slope.
Combined with aging, a just-spoken thought cannot be repeated
(\autoref{fig:im-dynamics}, panel~c). After about a minute it may become eligible again, but only if
\(s_{\mathrm{aged}}\) is still above \(\tau\).

\begin{figure}[H]
  \centering
  \input{assets/figures/04/raw/im-dynamics}
  \caption{Dynamics of the effective FIM score. (a)~Quadratic aging for three base scores, with
    linear and exponential kernels that share the same hold-time constraint for \(s_0=5\). The
    vertical line marks \(t_{\mathrm{hold}}\), where a score-5 thought crosses \(\tau\).
    (b)~Cooldown factor after a thought is spoken, compared with a linear ramp.
    (c)~Effective score of a thought with \(s_0=5\), never spoken versus spoken at
    \(t=12\,\mathrm{s}\). Recovery restores the aged remainder; whether that remainder still exceeds
    \(\tau\) depends on how long the thought has already lived.
    Parameters: \(\tau=4.3\), \(s_{\mathrm{hold}}=5\), \(t_{\mathrm{hold}}=45\,\mathrm{s}\).
    \figsource{Own illustration.}}
  \label{fig:im-dynamics}
\end{figure}
