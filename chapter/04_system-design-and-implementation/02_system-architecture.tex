...
\begin{itemize}
  ...
  \item Once a facilitation intervention has been spoken, repeating it immediately would over-talk and
    crowd the floor. The motivation to raise the same process issue is therefore initially very low, but
...
\(s_{\mathrm{eff}}\) assigns exactly one effective score. Aging and cooldown address the two
assumptions, so they multiply:
...
Linear is the simplest kernel that reaches zero in finite time and can be inverted from an
operational requirement. An exponential never reaches zero, so a residual could later interact with
cooldown recovery. Linear already lets weaker thoughts expire sooner, because they start closer to
\(\tau\) (\autoref{fig:im-aging}).

\(H\) is inverted, not fitted: a thought of score \(s_{\mathrm{hold}}\) must remain above \(\tau\)
for exactly \(t_{\mathrm{hold}}\) seconds.
Substituting \(s_{\mathrm{aged}}(s_{\mathrm{hold}}, t_{\mathrm{hold}})=\tau\) yields
...
\begin{figure}[H]
  \centering
  \input{assets/figures/04/raw/im-aging}
...
\begin{figure}[H]
  \centering
  \input{assets/figures/04/raw/im-cooldown}
...
The product in \eqref{eq:im-eff} is the result (\autoref{fig:im-effective}): a just-spoken thought
