...
start of the meeting if \(i\) has not spoken. The factor \(p>0\) and the thresholds
\(\theta_r>0\) and \(\theta_{\Delta}>0\) are configured.
...
A participant is invited in when their own speaking time is still below \(\theta_r\), they have
been silent for at least \(\theta_{\Delta}\), and they do not hold the floor:
\begin{equation}
  \mathrm{integrate}(i)
  \iff
  r_i < \theta_r
